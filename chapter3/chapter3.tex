...
\end{equation}
Um zusätzlich noch ein Maß für die Genauigkeit der geschätzten Position zu haben, wird der Mittelwert $\hat{a}$ der großen Achse $a_i$ der Unsicherheits-Ellipsen gebildet:
\begin{equation}
...
Der mittlere lange Durchmesser der Ellipse wächst bei sinkender Bildaufnahmefrequenz für alle Fahrkurven. Hier ist jedoch kein so signifikanter Sprung zu beobachten wie beim Abstand. Es lässt sich nur ein unterschiedlich starker Anstieg im Durchmesser für die Fahrkurven ausmachen.
\begin{description}
\item[Loop~Clockwise] zeigt den geringsten Anstieg und hat von allen Fahrkurven, bei den untersuchten Bildaufnahmefrequenzen, den kleinsten mittleren Durchmesser der Unsicherheits-Ellipse.
\item[Loop~Anticlockwise] hat in allen vier untersuchten Bereichen den zweit kleinsten Durchmesser. Sein Anstieg liegt nur knapp über dem von \textbf{Loop~Clockwise}.
\end{description}
...
\begin{description}
\item[Ellipse] Diese Fahrkurve liefert bei 500 und 1000\,$ms$ den größten Durchmesser. Der Anstieg ist dabei nicht so stark wie bei den  \textbf{Schlangenlinien}.
